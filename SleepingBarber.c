...
int tuttoFatto = 0;
int numClienti, numeroSedie;
void attesaRandom(int secondi);
...
int main(int argc, char * argv[]) {
  ...
  if (numClienti > CLIENTI_MASSIMI) {
    printf("IL NUMERO MASSIMO DI CLIENTI CHE POSSONO EFFETTUARE IL TAGLIO:  %d.\n", CLIENTI_MASSIMI);
    exit(-1);
  }
...
void * cliente(void * numero) {
  ...
  printf("IL CLIENTE N.%d : *** ENTRA NELLA SALA D'ATTESA. ***\n", num + 1);
  ...
  printf("IL CLIENTE N.%d : *** RISVEGLIA IL BARBIERE CHE STAVA DROMENDO. ***\n", num + 1);
  sem_post( & cuscinoBarbiere);
  ...
  printf("IL CLIENTE N.%d : *** LASCIA IL NEGOZIO, E SE NE VA. ***\n", num + 1);
}
...
void * barbiere(void * junk) {
  ...
  while (!tuttoFatto) {
    ...
    if (!tuttoFatto) {
      ...
      attesaRandom(2);
      printf("IL BARBIERE : *** HA FINITO IL TAGLIO ***\n");
      // Rilascia il thread cliente quando hai effettuato il taglio…
      sem_post( & taglio);
    } else {
      printf("IL BARBIERE : *** PUO' ANDARSENE. ***\n");
    }
  }
}
...
void schermoIniziale(){
	 printf("--------------------------- BENVENUTO -----------------------------\n");
	 printf("- Quest'applicazione gestisce il problema del barbiere dormiente  -\n");
	 printf("-------------------------------------------------------------------\n");
	 printf("- INSERISCI IL NUMERO DI CLIENTI (Max 20) :                       -");
	 scanf("%d", & numClienti);
